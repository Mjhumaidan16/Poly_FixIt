...
final class ViewRequestViewController: UIViewController {
    ...
    private func updateButtonStates(for status: String) {
        ...
        // Edit & Reassign → only enabled when Pending
        let canModify = (status == "Pending")
        EditButton.isEnabled = canModify
        
        let assignement: Set<String> = ["Accepted","Completed"]
        reas_assignButton.isEnabled = !assignement.contains(status)

        // Delete → disabled when Completed
        deleteButton.isEnabled = (status != "Completed")

        // Accept → disabled when already accepted / begun / completed
        let disableAcceptStatuses: Set<String> = ["Accepted", "Completed", "Begin"]
        acceptButtonTapped.isEnabled = !disableAcceptStatuses.contains(status)
    ...
    private func styleButton(_ button: UIButton) {
        button.alpha = button.isEnabled ? 2.0 : 0.8
    }
