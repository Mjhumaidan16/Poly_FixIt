...
struct StackedNotificationView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            Divider()
            Button(action: { print("Admin Control Panel Tapped") }) {
                Text("Control Panel")
    ...
    private func notificationRow(_ item: NotificationMessage) -> some View {
        HStack(alignment: .top, spacing: 10 * scaleFactor) {
            Circle()
                ...
                .overlay(Image(systemName: "shield.fill").font(.system(size: 10 * scaleFactor)).foregroundColor(.red))
...
class NotificationButton: UIBarButtonItem {
    ...
    private func setupUI() {
        triggerButton.setImage(UIImage(systemName: "bell.badge.fill"), for: .normal)
    ...
    private func fetchAllAdminNotifications() {
        db.collection("notifications")
            .order(by: "time", descending: true)
            .addSnapshotListener { [weak self] querySnapshot, error in
                guard let self = self, let documents = querySnapshot?.documents else { return }
                
                // Filter for references pointing to the "admin" collection
                let adminDocs = documents.filter { doc in
                    if let receiverRef = doc.data()["receiver"] as? DocumentReference {
                        return receiverRef.path.contains("admin")
                    }
                    return false
                }
                
                self.messages = adminDocs.map { doc in
                    let data = doc.data()
