...
final class EditRequestViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        // Debugging whether categoryButton is initialized here
          if categoryButton != nil {
              print("categoryButton is not nil in viewDidLoad.")
          } else {
              print("categoryButton is nil in viewDidLoad.")
          }
        setupPickers()
        setupImageTap()
        ...
        RequestManager.shared.fetchRequest(requestId: userId) { [weak self] fetchedRequest in
            ...
            if let request = fetchedRequest {
                ...
                print("Fetched request categories: \(request.category)")
    ...
    private func fetchCategoriesAndSetupMenu() {
        // Use the dynamic userId instead of the hardcoded "001"
        db.collection("requests").document("001").getDocument { [weak self] snapshot, error in
            ...
            print("Document data: \(data)")  // Print data to debug

            // Safely unwrap category field and set the categories array
            if let categories = data["category"] as? [String] {
                self.categories = categories

                // Debugging currentRequest and categories
                print("Fetched categories: \(categories)")
                print("currentRequest before update: \(String(describing: self.currentRequest))")

                // Make sure currentRequest is initialized before updating category
                if self.currentRequest == nil {
                    print("currentRequest is nil, initializing it.")
                    // Optionally initialize the request if it's nil
                    self.currentRequest = Request(document: snapshot!) // Make sure this initialization is correct
                }
                self.currentRequest?.category = categories // Update currentRequest with the fetched categories

                // Debugging updated currentRequest
                print("currentRequest after update: \(String(describing: self.currentRequest))")

                // Call the methods after making sure currentRequest is updated
                self.prefillCategory()
                self.setupCategoryMenu()
            } else {
                print("No 'category' field in document")
                self.categories = []  // Set empty array if 'category' is missing
            }
        }
    }
    ...
    private func prefillCategory() {
        // Debugging currentRequest category before accessing
        print("Prefill Category - currentRequest category: \(String(describing: currentRequest?.category))")

        // Ensure categoryName is not nil and retrieve its index in categories
        guard let categoryName = currentRequest?.category.first else {
            print("Category is nil or empty in currentRequest.")
            return
        }
        
        // Debugging the category being selected
        print("Prefilled Category Name: \(categoryName)")
        
        // Set the selected category to the category name
        selectedCategory = categoryName
        
        // Ensure categoryButton is initialized before setting its title
        if let categoryButton = categoryButton {
            categoryButton.setTitle(categoryName, for: .normal)
            print("Set categoryButton title to: \(categoryName)")
        } else {
            print("categoryButton is nil.")
        }
        
        // Debugging the selectedCategory
        print("Selected Category after setting: \(selectedCategory ?? "None")")
    }
