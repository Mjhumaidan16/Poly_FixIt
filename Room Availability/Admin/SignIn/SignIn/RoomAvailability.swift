...
final class LocationBackendViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    @IBAction func confirmSelectionButtonTapped(_ sender: UIButton) {
        ...
        _guardedSaveRoomAvailabilityCall()
    }
    ...
    private func saveRoomAvailability() {
        ...
        db.runTransaction({ (transaction, errorPointer) -> Any? in
            ...
            // ✅ NEW behavior:
            // If fully contained:
            // - If user did NOT confirm Proceed -> block (force them through the alert)
            // - If user DID confirm Proceed -> edit (shrink/replace) the containing interval to match the new interval
            ...
            for i in 0..<intervals.count {
                ...
                if fullyContained {
                    if self.allowContainedEdit {
                        // Replace (edit/shrink) the existing interval to the new one
            ...
            // If we edited a contained interval, we still want to keep data clean and merged if needed.
            // We’ll run the normal merge step afterwards, but starting from the updated intervals array.
            ...
            // If we did a contained edit, the “new interval” is already written into intervals[].
            // In that case, we don’t want to blindly add another merged interval duplicate.
            // We will only merge if there are overlaps that expand.
            if !didContainedEdit {
                // Standard merge/append path
    ...
    private func checkIfNeedsEditConfirmationAndProceedIfAllowed() {
        guard let docId = classLocation, !docId.isEmpty else { saveIsAllowedToRun = true; return }
        guard let s = startTime, let e = endTime else { saveIsAllowedToRun = true; return }
        if e <= s { saveIsAllowedToRun = true; return }
        ...
        docRef.getDocument { [weak self] snapshot, error in
            ...
            // ✅ NEW: We now prompt not only for “expand/merge”, but also for “contained edit”.
            ...
            for i in 0..<count {
                ...
                // If fully contained (example: 3:02->5:00 inside 3:00->5:00),
                // we DO want to ask for Proceed now.
                ...
                // Expands means it will modify availability (merge/expand) -> ask Proceed
            ...
            if shouldAskProceed {
                self.presentProceedOrCancelAlert(
                    ...
                    cancelTitle: "No",
                    onProceed: {
                        // User explicitly approved editing, including contained-shrink edits.
